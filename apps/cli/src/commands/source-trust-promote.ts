/**
 * `brain source trust promote <sourceId>` (Task 4.8/4.9) — the privileged trust promotion. Advances
 ...
 * AuthorizationChallenge. Output matches `source-trust-promote.schema.json`.
 */
...
import { promoteTrust, readTrustState } from "../trust/index.js";
...
function parseArgs(argv: string[]): Parsed {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a.startsWith("-")) throw CliError.usage(`\`source trust promote\`: unknown flag ${a}`);
    else if (sourceId === undefined) sourceId = a;
    else throw CliError.usage(`\`source trust promote\`: unexpected argument ${a}`);
  }
  if (sourceId === undefined) throw CliError.usage(`\`source trust promote\`: expected a <sourceId> argument`);
  return { sourceId, exportChallenge, ...(authorization !== undefined ? { authorization } : {}) };
}

async function sourceTrustPromote(ctx: RunContext): Promise<number> {
  ...
  try {
    ...
    const op = { op: "source trust promote", canonicalBaseCommit: trustLedgerHead, intendedEffect: { kind: "trust" as const, sourceOpaqueId: p.sourceId, fromLevel: current.level, toLevel: "trusted" } };
    ...
    try {
      ...
      const next = await promoteTrust(target, "trusted", `promote via ${p.sourceId}`, {
        db: store.db,
        advanceTrustLedger: async () => { await client.execAuthorized(op, authorization); },
      });
      const out = { command: "source trust promote", sourceId: p.sourceId, rawContentHash: target.rawContentHash, trustState: next.level, trustLedgerHead: (await repo.readRef(TRUST_REF)) ?? trustLedgerHead };
      if (ctx.output.mode === "json") emitJson(out);
      else ctx.render(`promoted ${p.sourceId} → ${next.level}`);
      return EXIT.OK;
    } finally { client.close(); }
  } finally {
    store.close();
...
registerCommand("source trust promote", sourceTrustPromote);

export { sourceTrustPromote };
