...
export async function buildSyncPlan(
  ...
  for (let i = 0; i < commits.length; i++) {
    ...
    // The commit's path/oid contribution reaches the signed audit ref via the
    // sync intent detail — Atlas-generated bytes, scanned BEFORE any absorb of
    // this commit (a secret-bearing filename halts the cycle, exit 3).
