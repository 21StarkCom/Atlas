/**
 * `source-trust-promote.cli.e2e` (Task 4.8/4.9) — the `brain source trust promote` authorization
 * gate over the REAL broker daemon: a captured source with `--export-challenge` emits a broker
 * AuthorizationChallenge (op `source trust promote`) + exits 6; without an authorization it is
 * action-required (exit 6); an unknown source is rejected (exit 1). The authorized→promoted path's
...
describe("brain source trust promote (authorization gate)", () => {
  it("--export-challenge emits a challenge (op source trust promote) + exits 6", async () => {
    const r = await cli(["source", "trust", "promote", SOURCE_ID, "--export-challenge", "--json"]);
    expect(r.code, r.out).toBe(6);
    const challenge = JSON.parse(r.out);
    expect(challenge.op).toBe("source trust promote");
    expect(challenge.nonce).toBeTruthy();
  });

  it("without an authorization it is action-required (exit 6)", async () => {
    const r = await cli(["source", "trust", "promote", SOURCE_ID, "--json"]);
    expect(r.code).toBe(6);
  });

  it("an unknown source is rejected (exit 1)", async () => {
    const r = await cli(["source", "trust", "promote", `sha256:${"c".repeat(64)}:text/plain`, "--export-challenge", "--json"]);
