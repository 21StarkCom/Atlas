...
export function loadBrokerConfigFromEnv(env: NodeJS.ProcessEnv = process.env): BrokerServiceConfig {
  ...
  // v2: canonical is always the fixed default set — no config/env indirection. Only
  // `audit`/`trust` remain fixed anchors alongside the single canonical branch.
  const refs = DEFAULT_PROTECTED_REFS;
