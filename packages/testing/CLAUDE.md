...
- **`ATLAS_PROVISIONED=1`** gates suites needing the real two-UID identity + file-key custody + WORM-anchor layout. CI sets it **after** `sudo -E provisioning/ci/setup.sh`, so those suites **never skip in CI** (`.github/workflows/ci.yml:43`). Without it locally, tests exercise an in-process subset (in-process `BrokerService` + local git fixture vault).
