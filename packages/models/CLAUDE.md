...
- `mintEgressCapability(run: RunBinding, limits: EgressLimits, opts?)` — `opts` (`secret`/`keyId`/`now`/`nonce`) is TESTS-ONLY; production resolves the secret from custody. Constants: `CAPABILITY_KEY_ENV = "ATLAS_EGRESS_CAPABILITY_KEY"`, `DEFAULT_CAPABILITY_KEY_ID = "atlas-egress-cap-v1"`, `DEFAULT_CAPABILITY_TTL_SECONDS = 300` (matches the broker nonce TTL).
