...
- **No-protected-write (the core invariant).** Every ref-*writing* path routes through `updateAgentRef` / `deleteAgentRef` / `attachHeadToAgentRef`, each of which calls `assertAgentRef` FIRST (`refs.ts:37-44` rejects anything not matching `refs/agent/<ulid>`). Enforced three independent ways in `git.no-protected-write.test.ts`:
...
- **Deterministic agent authorship.** Commits use `Atlas Agent <agent@atlas.local>` set per-invocation via `-c`, with `commit.gpgsign=false` — never mutates ambient config (`worktree.ts:17-18,40-60`). Distinct from the human `Aryeh Stark <aryeh@21stark.com>` the broker stamps on audit commits (`packages/broker/src/git.ts:44-47`).
