...
- **Two disjoint seq spaces on the shared `audit_events.seq` PK.** `run.*` events (git-anchored, gapless `seq == last+1`) grow from 0. Ledger-internal D6 events carry no git event, so they allocate from a disjoint HIGH range `DB_EVENT_SEQ_BASE = 1_000_000_000_000` (`nextDbEventSeq` counts all non-`run.*` events, incl. `evidence.retry_enqueued` — narrowing to `db.%` would collide two ledger-internal kinds, #139). **Every RUN-space reader partitions by the RANGE (`seq < DB_EVENT_SEQ_BASE`), never by event-type prefix** — a `NOT LIKE 'db.%'` filter counts `evidence.retry_enqueued` into the run space, after which `nextRunSeq()` allocates ~10^12+n and EVERY broker-anchored run is refused `broker.audit_seq_nonmonotonic` (one `evidence retry` bricked all subsequent runs; found live by the #217 reverify E2E, fixed in `nextRunSeq`/`latestRunSeq`/`readCoalesceCovers` + the CLI's `anchor-check`; pinned by `test/ledger/seq-partition.test.ts`).
