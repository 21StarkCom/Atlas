...
- [ ] **Step 3: Implement** — in `apps/cli/src/config/schema.ts`, add to `GitConfig` (`:94-100`): `canonical_ref: z.string().min(1).default("refs/heads/main")`. Add to `VaultConfig` (`:18-22`): `note_globs: z.array(z.string().min(1)).min(1).default(["**/*.md"])`.
...
**Rollback (Phase 1):** Revert the branch. The `0012` migration is forward-only but inert if unused (empty table); a rolled-back config falls back to `refs/heads/main` default. No data migration to undo.
...
- [ ] **Step 1: Write the failing test** — `apps/cli/test/sync-autohook.test.ts`: drive the wrapper's exact sequence (`brain sync --json` then `brain jobs run --workflow index:reconcile --json`) over a committed upstream note change; assert the change is retrievable afterward (chunks re-embedded, `active_generation_id` current) within the single wrapper invocation. Break: the chain drops the enqueued job (sync advances, reconcile never drains).
- [ ] **Step 2: Run — expect FAIL.**
- [ ] **Step 3: Implement** — `provisioning/macos/atlas-sync-wrapper.sh` (`set -euo pipefail`): fetch `ATLAS_EGRESS_CAPABILITY_KEY` from Keychain (`security find-generic-password -w -s atlas-egress-capability -a atlas-agent`), export it for the two `brain` invocations only, run `brain sync --json`, then `brain jobs run --workflow index:reconcile --json`. Never write the key to disk/temp/audit.
- [ ] **Step 4: Run — expect PASS.**
