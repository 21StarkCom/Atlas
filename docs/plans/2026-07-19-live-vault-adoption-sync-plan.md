...
- Modify: every existing hardcoded `.md` note selector to route through the shared matcher (rebuild note-discovery `packages/sqlite-store/src/rebuild.ts`, capture/ingest discovery, and any corpus scan) — so sync-diff, reset, rebuild, and capture agree on one inclusion rule
...
- [ ] **Step 5: Create the shared matcher + rewire every `.md` selector** — `apps/cli/src/vault/note-matcher.ts` exporting `matchesNoteGlobs(path, globs)` and `noteGlobPathspec(globs)` (git pathspec for diff/diff-tree). Replace the hardcoded `.md` filter in rebuild note-discovery, capture/ingest discovery, and any corpus scan with calls to it, so sync-diff, reset, rebuild, and capture share one inclusion rule. Add `apps/cli/test/note-matcher.test.ts` + a non-default-glob E2E (`notes/**/*.md`) asserting a `docs/x.md` is uniformly **excluded** by both an incremental sync diff and a subsequent `db rebuild` (no divergent projections).
...
- Create: `packages/sqlite-store/src/note-derivation.ts` (shared `deriveAndUpsertNote` primitive), `packages/sqlite-store/src/fold-notes-for-paths.ts` (exports `foldNotesForPaths`)
- Modify: `packages/sqlite-store/src/rebuild.ts` (call the shared `deriveAndUpsertNote` instead of the inline `189-203` derivation)
...
- [ ] **Step 2: Run — expect FAIL** (module missing).
- [ ] **Step 3: Implement — extract ONE shared derivation primitive; do not fork rebuild's rule.** First extract the per-note derivation currently inline in `rebuildProjections` (`rebuild.ts:189-203` — lifecycle, `content_hash`, `active_generation_id`, archival) into a shared `deriveAndUpsertNote(store, repo, ref, noteId)` in `packages/sqlite-store/src/note-derivation.ts`, and **rewire `rebuildProjections` to call it** so full rebuild and incremental sync run the *same* rule (a later change to one can never silently diverge the projection). Then `packages/sqlite-store/src/fold-notes-for-paths.ts` iterates `noteIds` and calls that primitive, deriving each note's presence from `ref` (read the ref tree, resolve each `noteId`'s path/blob). Two outcomes per `noteId`:
  - resolves at `ref` → active upsert via the shared primitive (idempotent, keyed by `note_id`);
...
- [ ] **Step 4: Run — expect PASS.** Add a **parity test** (`packages/sqlite-store/test/fold-rebuild-parity.test.ts`): for the same canonical tree, a full `rebuildProjections` and an incremental `foldNotesForPaths` over the union produce **byte-identical** `notes` rows (every column) across active, archived, and re-added notes.
...
- [ ] **Step 3: Implement** — **first extract a side-effect-free `planCaptureSource` primitive** in `apps/cli/src/ingest/capture.ts`: it accepts **bytes read from an exact `commit:path`** (via a new `repo.readBlobAt(commitOid, path)` — never the working tree, so a concurrent worktree edit can't leak in) plus source metadata, runs exactly the existing normalization + scan preflight, and **returns proposed ops + observation metadata without opening or integrating a run**. Refactor `captureSource` to call `planCaptureSource` then integrate, so both share one pipeline. Then in `cycle.ts` dispatch, over the bytes at the snapshotted commit — threading a per-note in-memory overlay so that when one note changes across multiple commits in the range, each later snapshot is planned against that note's **already-planned** state (never the stale pre-run canonical), collapsing the sequence into one valid final op per note while every intermediate version is still scanned:
  A **path→noteId resolver** underpins D/R: a deleted/renamed-away path has no blob in the current upstream tree, so its note ID is resolved from the **old** canonical tree (`config.git.canonical_ref` before this cycle) + stored provenance, not from the vanished path — dispatch returns explicit impacted note IDs for **every** A/M/D/R outcome (both sides of a rename), so filenames that differ from note IDs still archive/re-index the right note.
  ...
  - `D`: resolve the path in the old canonical tree first. If it maps to an absorbed note → append `ProposeArchive` (`packages/contracts/src/ops/archive.ts`) for that note ID; **if the path was only ever quarantined (pending-only, never absorbed into canonical)** → emit **no** content op (there is no note/content to archive) and just clear its pending entry — never append an archive with no canonical source (that would fail planning and wedge the cursor). Mark prior pending for removal.
...
- [ ] **Step 1: Write the failing tests** — split across `sync-dry-run.test.ts` and `sync-max-paths.test.ts` and `sync-cycle.test.ts`: non-attributable `GeneratedArtifactGuard` verdict → exit 3, cursor unadvanced; non-terminal per-path error → exit 4 `retryable:true`, cursor + pending unchanged; `--dry-run` mutates nothing (cursor/`cycle_seq`/pending unchanged, `quarantineId:""`, `clearedPending` planned-only, `appliedOps:0`, `reconcileJobId:null`, `cursorFrom==cursorTo`); `--max-paths n` advances cursor to the last fully-processed commit boundary (`truncated:true`), atomic oversize commit processed in full, second unbounded cycle resumes to head with every path absorbed once; **deterministic exit-3 does not permanently wedge** — an offending historical commit followed by later clean commits: repeated `sync` deterministically returns exit 3 (cursor stuck), `sync status` surfaces the blocking commit OID + reason under a new `blocked` field, and an operator-authorized `sync reset` (Task 5.3, applicable to a deterministic exit-3 even when ancestry is intact) re-converges to the current upstream tree and clears the block so the following `sync` reaches head.
- [ ] **Step 2: Run — expect FAIL.**
- [ ] **Step 3: Implement** — the exit-3 branch (abort, cursor unadvanced) for `GeneratedArtifactGuard`, **recording the blocking commit OID + reason in durable state and surfacing it in `sync status` as `blocked: { commitOid, reason } | null`** so the wedge is observable and the operator knows to run `sync reset` (a deterministic exit-3 is a valid `sync reset` trigger, not only divergence — Task 5.3/5.4 document this recovery); the exit-4 branch (abort on non-terminal per-path error, no partial finalize); `--dry-run` (compute + classify + scan preflight, mutate nothing, open no run); `--max-paths` (per-commit cumulative count check, advance to last fully-processed commit OID, atomic commit).
- [ ] **Step 4: Run — expect PASS.**
...
- [ ] **Step 2: Run — expect FAIL.**
- [ ] **Step 3: Implement** — `provisioning/macos/atlas-sync-wrapper.sh` (`set -euo pipefail`): run `brain sync --json` **with no egress credential in its environment** — `sync` only scans + integrates untrusted upstream content and never mints egress, so the long-lived capability key must not be inherited by the larger untrusted-content phase (keeping the network-denied `atlas-agent` boundary tight and shrinking crash-dump/RCE blast radius). Capture its exit code without tripping `set -e` (`set +e; brain sync --json; rc=$?; set -e`). **Only when `rc` is 0 or 6** (the success-envelope outcomes that advanced the cursor and enqueued reconcile) fetch `ATLAS_EGRESS_CAPABILITY_KEY` from Keychain (`security find-generic-password -w -s atlas-egress-capability -a atlas-agent`) and pass it **command-scoped to the drain only** (`ATLAS_EGRESS_CAPABILITY_KEY="$key" brain jobs run --workflow index:reconcile --json`), then `unset` it immediately after that process exits; exit non-zero without draining for any true-abort status (2/3/4). Never write the key to disk/temp/audit. The Step-1 test asserts the `brain sync` subprocess environment does **not** contain the credential.
- [ ] **Step 4: Run — expect PASS.**
...
- [ ] **Step 2: Run — expect FAIL.**
- [ ] **Step 3: Implement** — ensure the mint path fails closed on absent/locked key (never falls back to a credential-less egress). **Define revocation as broker-side invalidation of the old key at `atlas-egress` plus removal of the agent copy — not Keychain deletion alone.** Specify an **executable, recoverable rotation protocol** with a single source of truth and staged ordering: (1) disable the sync timer; (2) retain the old value for rollback; (3) install/stage the new key at the `atlas-egress` broker (the authority); (4) update + verify the `atlas-agent` Keychain item; (5) run an end-to-end mint/reconcile probe with the new key; (6) invalidate/retire the old key at the broker and confirm the old value is now rejected; (7) re-enable the timer. Rollback is defined for either half-applied state (agent-updated/broker-not, or broker-updated/agent-not) — restore the retained old value and re-probe. The **install gate verifies both custody points are populated and mutually valid** (a successful mint), not merely that the Keychain is unlocked. **Flag the CI parity gap** (the real Keychain-fetch + two-custody-point rotation is not exercised by CI — verified only by the live-drive runbook).
