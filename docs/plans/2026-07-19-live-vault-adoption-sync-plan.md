...
- [ ] **Step 5: Create the shared matcher + rewire every `.md` selector** — `apps/cli/src/vault/note-matcher.ts` exporting `matchesNoteGlobs(path, globs)` and `noteGlobPathspec(globs)` (git pathspec for diff/diff-tree). Replace the hardcoded `.md` filter in rebuild note-discovery, capture/ingest discovery, and any corpus scan with calls to it, so sync-diff, reset, rebuild, and capture share one inclusion rule. Add `apps/cli/test/note-matcher.test.ts` + a **`db rebuild`** non-default-glob test (`notes/**/*.md`) asserting a `docs/x.md` is **excluded** from the rebuilt projection (the matcher + rebuild half — both exist in Phase 1). *(The incremental-**sync** half of this equivalence — that the same `docs/x.md` is also excluded by a `runSyncCycle` diff, proving rebuild and sync agree — is added in Phase 4 Task 4.10, once `runSyncCycle` exists.)*
...
- Create: `packages/sqlite-store/src/note-derivation.ts` (shared `deriveAndPersistNote(store, facts)` primitive over `{ noteId, path, blob }`), `packages/sqlite-store/src/fold-notes-for-paths.ts` (exports `foldNotesForPaths`)
- Modify: `packages/sqlite-store/src/rebuild.ts` (per-note loop body calls the shared `deriveAndPersistNote`; `rebuildProjections(store.db, snapshot)` signature unchanged)
...
- [ ] **Step 2: Run — expect FAIL** (module missing).
- [ ] **Step 3: Implement — extract ONE shared derivation primitive over per-note facts; do not fork rebuild's rule, and do not change rebuild's public signature.** Extract the per-note derivation currently inline in `rebuildProjections` (`rebuild.ts:189-203` — lifecycle, `content_hash`, `active_generation_id`, archival) into a shared `deriveAndPersistNote(store, facts: NoteDerivationFacts)` in `packages/sqlite-store/src/note-derivation.ts`, where `NoteDerivationFacts = { noteId, path, blob | null }` (blob `null` ⇒ archived) — the note facts **both callers already compute**: `rebuildProjections` from its in-memory `VaultSnapshot` (so `rebuildProjections(store.db, snapshot)` keeps its exact current signature — Task 1.4's call is unaffected), and `foldNotesForPaths` from the ref tree. **Rewire the rebuild loop's per-note body to call the primitive** (not its signature) so both run the *same* rule and can never silently diverge. Then `packages/sqlite-store/src/fold-notes-for-paths.ts` iterates `noteIds`, reads each note's path/blob from `ref`, and calls `deriveAndPersistNote`. Two outcomes per `noteId`:
