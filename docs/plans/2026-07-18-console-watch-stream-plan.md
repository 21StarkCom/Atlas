...
2. **Refactor the jobs-list item projection at its owner (`@atlas/jobs`)**
   - What: `apps/cli/src/commands/jobs.ts` does not own the projection — `packages/jobs/src/repo.ts` does. Ensure `JobListRow` carries `updatedAt` (add the field + select `updated_at` in `listJobs` if absent) and export a **shared row projector** `projectJobListRow(row): JobListRow` so both `listJobs` and `watch` produce the identical shape. **Byte-stability, resolved honestly (not a contradiction):** first *inspect* `repo.ts` to settle whether `JobListRow` already has `updatedAt`. If it does, `jobs list --json` output is genuinely unchanged (byte-identical golden holds). If it does **not**, adding the field to the SSOT projection necessarily adds one key to `jobs list --json` — that is an **intentional, reviewed additive change**: regenerate the golden, and the *only* permitted diff is the single new `updatedAt` field on each row (asserted below). The plan does **not** claim byte-identity in the field-adding case; it claims a one-field additive diff. (Splitting the shape so only `watch` carries `updatedAt` is rejected — it would fork the SSOT projection.) `jobs.ts` continues to serialize `listJobs`' (paginated `{limit, offset}`) rows to JSON. `watch`'s `job` event is `JobListRow` field-for-field (it already includes `updatedAt`; the §7.3 payload = `jobId, workflow, state, attempts, maxAttempts, nextRunAt?, lastError?, updatedAt`).
...
4. **Read-only ledger opener in `@atlas/sqlite-store`**
   ...
   - Interfaces — **Produces (from `@atlas/sqlite-store`):** `function openReadonlyLedger(path: string): SqliteDatabase`; `function ledgerSchemaState(db: SqliteDatabase): "ready" | "absent"`; **a package-root re-export of `DB_EVENT_SEQ_BASE`** (owned by `intents.ts`) so the CLI's Phase 4 low/high-space split consumes the constant from its owner — never a copied `1_000_000_000_000` literal; re-export of the `SqliteDatabase` type.
...
4. **Drift assertions: schema enum ↔ DDL CHECK, space predicate ↔ `intents.ts`, and `job` member ↔ `JobListRow`**
   - What: a test pinning `watch.schema.json`'s `audit.eventType` enum **derived at runtime** from the `audit_events` CHECK (parsed from the migration source, not hardcoded) so a new DDL kind fails CI until the schema enum matches; asserting the low/high classification predicate resolves to `run.%` / non-`run.%` with the `DB_EVENT_SEQ_BASE` threshold consistent with `intents.ts`; **and a `job`-member drift assertion — the field set of `watch.schema.json`'s `job` event must equal the key set of `@atlas/jobs`' `JobListRow` (the SSOT shape), so a later `JobListRow` field add/rename fails CI until the schema replica matches** (the same owner-to-replica guard the audit enum gets — the job schema is not left un-pinned). The Phase 6 sweep's `watch` adapter also emits at least one `job` event under `--once`-style seeding so the replica is exercised, not just structurally compared.
...
2. **Atomic attach: resolve probe, then synchronous consistent capture — capturing the immutable replay window inside the transaction**
   ...
   - Interfaces — **Produces:**
     ...
     interface LedgerIdentity { device: number; inode: number; schemaHead: string }  // device/inode from fstat on the OPEN connection's fd — the bytes actually being read — never a separate stat(path) that an atomic restore could point at a different file
