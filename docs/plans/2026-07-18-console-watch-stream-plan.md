...
- **Broker-failure taxonomy (grounded in `protocol.ts`, single authority):** one shared `isTransportError` classifier (`apps/cli/src/health/socket-errors.ts`, Phase 1 Task 3a) decides socket unreachability for **both** the anchor probe and the daemon probe — no second list. A transport/socket error **or** RPC timeout **or** any other unexpected throw ⇒ `unreachable` ⇒ degrade to `anchorSource:"sqlite-only"` (the resolver is total). Only a refusal matched by `@atlas/broker`'s exported `isBadRequestRefusal` (the `"broker.bad_request"` code, owned by the broker — never a CLI literal) ⇒ `protocol-error` ⇒ `watch` fatal exit 4, `status` degrades unchanged.
...
**Goal:** Extract snapshot derivations into a **synchronous** shared function fed by a **pre-resolved, typed** broker probe classified against the real broker error contract; refactor the jobs-list projection at its owning package; and expose a read-only ledger opener from `@atlas/sqlite-store`. No behavior change to `status`/`doctor`; the **only** intended `jobs list --json` change is the single additive `updatedAt` field (confirmed absent from `JobListRow` today — `repo.ts:719` — and required by the `job` event's SSOT reuse), a reviewed one-field diff.
**Dependencies:** none
**Estimated effort:** M
...
1. **Split the broker probe (broker-contract-grounded) from a synchronous snapshot derivation**
   - What: two coordinated changes in `apps/cli/src/audit/anchor-check.ts` + a new `apps/cli/src/health/snapshot.ts`.
     - **(a) Typed, async probe resolver, split from the synchronous verdict.** In `anchor-check.ts`, factor the RPC out of `verifyAuditAnchor`. Add `resolveAnchorProbe(broker: AuditChainProbe | null, env: NodeJS.ProcessEnv): Promise<AnchorProbe>` (async, the **only** async part) and `deriveAnchorVerdict(db: SqliteDatabase, anchorPath: string, env: NodeJS.ProcessEnv, probe: AnchorProbe): AnchorCheckResult` (**synchronous** — it runs the existing SQLite cross-check `live.count`/`live.head` vs the probe's `count`/`head` against `db` inside the caller's transaction). `resolveAnchorProbe` classifies using the **actual broker contract** in `packages/broker/src/protocol.ts`, wrapping the RPC in a bounded timeout so an *ignored malformed frame* (which never correlates a response) cannot hang the stream:
       - `broker === null`, or the call throws any **transport/socket error** — classified by the **single shared `isTransportError(err)` predicate** (Task 3a's `apps/cli/src/health/socket-errors.ts`), which covers the full connect-failure set the socket layer can emit (`ECONNREFUSED`, `ENOENT`, `EPIPE`, `ECONNRESET`, `EACCES`, `ETIMEDOUT`, plus the Node `SystemError` connect shapes) — or the RPC timeout (`ATLAS_WATCH_PROBE_TIMEOUT_MS`, default 2000 ms) elapses before a correlated response ⇒ `{kind:"unreachable"}` (a hung or unreachable broker is an outage → degrade).
       - the call throws a **malformed-correlated-result refusal**, identified by the discriminant **exported from `@atlas/broker`** (`isBadRequestRefusal(err)` — never a `"broker.bad_request"` string literal copied into the CLI; the broker owns the code) ⇒ `{kind:"protocol-error", detail: err.code}`.
       - a correlated, well-formed `AuditChainStatus` result ⇒ `{kind:"answered", status}`.
       - **any other unexpected throw** — the resolver is **total** — ⇒ `{kind:"unreachable"}`: fail-safe to the degrading path so `status` never regresses from its pre-refactor catch-all degradation. Only an `isBadRequestRefusal` throw is ever `protocol-error`; every non-answered, non-refusal outcome degrades.
   ...
   - Interfaces — **Produces:**
     ...
     ```
     **Consumes:** the existing backup/watermark readers and `liveAudit(db)` cross-check inside `deriveAnchorVerdict`; the `AuditChainProbe`/`AuditChainStatus` types and the **`isBadRequestRefusal(err)` discriminant exported from `@atlas/broker`** (the owner of the `BrokerRefusal` class + its code — the CLI never re-declares the code literal); the shared `isTransportError` predicate from `apps/cli/src/health/socket-errors.ts` (Task 3a).
   - Test (create + modify): `apps/cli/test/read-commands.cli.test.ts` stays green; add a golden assertion that `status --json` is byte-identical to a pre-refactor capture for the fixture vault. New `apps/cli/test/anchor-probe.test.ts` — with a broker stub that (i) refuses connection ⇒ `unreachable`; (ii) throws the broker's real `broker.bad_request` refusal (constructed via `@atlas/broker`, matched by `isBadRequestRefusal`) ⇒ `protocol-error`; (iii) never responds (ignored-frame simulation) ⇒ `unreachable` via the timeout; (iv) returns a valid status ⇒ `answered`; (v) **`ECONNRESET`, `EACCES`, and `ETIMEDOUT` transport errors each ⇒ `unreachable`** (the totality/socket-set proof); (vi) **an arbitrary unexpected exception ⇒ `unreachable`** (resolver totality); and `deriveAnchorVerdict` maps both `unreachable` and `protocol-error` to `sqlite-only` (the `status`-preserving assertion). **Drift guard:** the `protocol-error` case asserts against a `BrokerRefusal` produced by `@atlas/broker` itself (not a hand-built literal), so a broker-side code rename fails this test rather than silently reclassifying to `unreachable`.
2. **Refactor the jobs-list item projection at its owner (`@atlas/jobs`)**
   - What: `apps/cli/src/commands/jobs.ts` does not own the projection — `packages/jobs/src/repo.ts` does. Ensure `JobListRow` carries `updatedAt` (add the field + select `updated_at` in `listJobs` if absent) and export a **shared row projector** `projectJobListRow(row): JobListRow` so both `listJobs` and `watch` produce the identical shape. **Byte-stability, settled by inspection (not a contradiction):** `JobListRow` (`repo.ts:719`) does **not** carry `updatedAt` today, and `listJobs`' `SELECT` (`repo.ts:746`) does not read `updated_at` — the pre-existing `updatedAt` lives only on the *enqueue-row* projection (`repo.ts:78`), a different shape. So adding the field to the SSOT list projection necessarily adds one key to `jobs list --json`: an **intentional, reviewed additive change**. Regenerate the golden; the *only* permitted diff is the single new `updatedAt` field on each row (asserted below). The plan therefore does **not** claim byte-identity for `jobs list --json` — it claims a one-field additive diff, and the Phase 1 goal + verification say exactly that. (Splitting the shape so only `watch` carries `updatedAt` is rejected — it would fork the SSOT projection.) `jobs.ts` continues to serialize `listJobs`' (paginated `{limit, offset}`) rows to JSON. `watch`'s `job` event is `JobListRow` field-for-field (it already includes `updatedAt`; the §7.3 payload = `jobId, workflow, state, attempts, maxAttempts, nextRunAt?, lastError?, updatedAt`).
     - **Unpaginated full-table reader for `watch`.** `listJobs` is paginated (`{limit, offset}`, CLI-capped at 500/page) — `watch`'s full-table diff (§9.1, §4 scale of 10²–10³ jobs) must see **every** job in one **transactionally consistent** read, not a page. **Single query, two entry points — never two SELECTs.** Extract one internal raw-row query builder `jobListRawQuery(db, opts?: { limit: number; offset: number })` in `repo.ts` that owns the sole `SELECT`/`JOIN`/`WHERE`/`ORDER BY` (pagination is an *optional* `LIMIT/OFFSET` tail on that one statement). Both `listJobs` (with `{limit, offset}`) and `listAllJobs(db: SqliteDatabase): JobListRow[]` (no limit/offset, wrapped in a single `db.transaction(() => …)`) are implemented **through that one builder**, then map every row via `projectJobListRow` (one owner for both the SQL and the shape). A later change to filters, joins, or selected columns therefore lands in the builder and reaches both readers at once — the two cannot fork. `watch` consumes `listAllJobs`; `jobs list --json` keeps `listJobs`. This closes the "jobs beyond the selected page are never observed" gap without a pagination loop in the poller.
   ...
   - Test (create + keep): new `packages/jobs/test/repo.projection.test.ts` — `listJobs` rows include `updatedAt` and equal `projectJobListRow` applied to the raw rows; **`listAllJobs` on a seeded set of 600 jobs (past the 500 page cap) returns all 600**, each equal to `projectJobListRow` of the raw row, in one consistent read; **overlap assertion — `listAllJobs(db)` equals the concatenation of `listJobs` paginated across the full range on the same snapshot** (identical rows, identical order), proving both entry points resolve through the one `jobListRawQuery` builder and cannot fork; `apps/cli/test/jobs.cli.test.ts` green (if `updatedAt` pre-existed, byte-identical to the old golden; if newly added, the golden is regenerated and a diff assertion proves the **only** change is the single additive `updatedAt` field per row — no reordering, no other field moved).
...
3a. **One shared socket-error classifier + a broker-owned refusal discriminant (single authority for the taxonomy)**
   - What: eliminate the two competing socket-code lists and the copied refusal literal the taxonomy would otherwise fork into.
     - **(a) Shared transport-error classifier.** New `apps/cli/src/health/socket-errors.ts` exporting `isTransportError(err: unknown): boolean` — the **single** predicate both `resolveAnchorProbe` (Task 1) and `probeDaemon` (Task 3) consume to decide "ordinary socket unreachability." It recognizes the complete connect-failure set the daemon socket layer can emit: `ECONNREFUSED`, `ENOENT`, `EPIPE`, `ECONNRESET`, `EACCES`, `ETIMEDOUT` (matched on `err.code` of a Node `SystemError`, and unwrapping `AggregateError`). Operation-specific outcomes (the anchor probe's RPC **timeout**, a post-connect `EPIPE`) are layered on top **by each caller**, not duplicated into this base. This is the one authority; neither probe path keeps its own list.
     - **(b) Broker-owned refusal discriminant.** Add `isBadRequestRefusal(err: unknown): err is BrokerRefusal` to `packages/broker/src/protocol.ts` (exported from `@atlas/broker`), owning the check against the `"broker.bad_request"` code **inside the package that throws it**. The CLI's `resolveAnchorProbe` and its tests consume this predicate — the code literal never leaves `@atlas/broker`. A broker-side rename updates one place and the CLI follows.
   - Files: new `apps/cli/src/health/socket-errors.ts`; `packages/broker/src/protocol.ts` (+ `packages/broker/src/index.ts` export).
   - Interfaces — **Produces:** `isTransportError(err: unknown): boolean` (from `apps/cli`); `isBadRequestRefusal(err: unknown): err is BrokerRefusal` (from `@atlas/broker`).
   - Test (create): `apps/cli/test/socket-errors.test.ts` — the full code matrix (`ECONNREFUSED`/`ENOENT`/`EPIPE`/`ECONNRESET`/`EACCES`/`ETIMEDOUT` ⇒ true; an arbitrary error ⇒ false; an `AggregateError` wrapping a transport code ⇒ true); `packages/broker/test/refusal-discriminant.test.ts` — `isBadRequestRefusal` returns true for the broker's own thrown `broker.bad_request` refusal and false for any other error, pinning the discriminant to the real thrown shape.
4. **Read-only ledger opener in `@atlas/sqlite-store`**
   - What: add `openReadonlyLedger(path: string): SqliteDatabase` to `packages/sqlite-store/src/connection.ts` (better-sqlite3 `readonly: true`, no migration/checkpoint/write) and export it from `src/index.ts`. `apps/cli` consumes this opener + the already-exported `SqliteDatabase` type — it never imports `better-sqlite3` directly (which is not in its dependency graph). This is the interface Phase 3's attach and every `SqliteDatabase`-typed `watch` module use. **Schema-readiness, not just path presence:** the opener also exposes `ledgerSchemaState(db: SqliteDatabase): "ready" | "absent"` — `"absent"` when the `audit_events` table (or the migration-version row) is missing, i.e. the file exists but migrations have not completed. `watch`'s attach (Phase 3 Task 2) treats `"absent"` as **detached** (poll again), never as a fatal missing-table exception mid-snapshot.
     - **Identity primitive lives here too — apps/cli never touches an fd.** Because the store owns `better-sqlite3` **and** `fs`, it exposes `captureLedgerIdentity(db: SqliteDatabase): LedgerIdentity` (device + inode via `fs.fstatSync` on the connection's own open descriptor — the store has the descriptor; `apps/cli` does not — plus the schema head). The CLI receives a typed `LedgerIdentity` value, so the FD-based, restore-safe capture is real and owner-side, and `apps/cli` still takes no `better-sqlite3` dependency. `LedgerIdentity` is exported from `@atlas/sqlite-store`.
   - Files: `packages/sqlite-store/src/connection.ts`, `packages/sqlite-store/src/index.ts`.
   - Interfaces — **Produces (from `@atlas/sqlite-store`):** `function openReadonlyLedger(path: string): SqliteDatabase`; `function ledgerSchemaState(db: SqliteDatabase): "ready" | "absent"`; `function captureLedgerIdentity(db: SqliteDatabase): LedgerIdentity` (fd-based, defined + owned here); `interface LedgerIdentity { device: number; inode: number; schemaHead: string }`; **a package-root re-export of `DB_EVENT_SEQ_BASE`** (owned by `intents.ts`) so the CLI's Phase 4 low/high-space split consumes the constant from its owner — never a copied `1_000_000_000_000` literal; re-export of the `SqliteDatabase` type.
   - Test (create): `packages/sqlite-store/test/readonly-open.test.ts` — the opener returns a usable read connection on a seeded ledger; a write attempt through it throws `SQLITE_READONLY`; opening a missing path throws a distinguishable error (so `watch`'s attach can map absence → `ledger.attached:false`); **a file that exists but has no migrated schema returns `ledgerSchemaState === "absent"`** (the created-but-not-yet-migrated race), and a fully-migrated ledger returns `"ready"`; **`captureLedgerIdentity` reflects the open fd, not the path — after an atomic replace of the path with a different file, the identity of the still-open connection is unchanged** (the restore-safety property).
...
4. **Drift assertions: schema enum ↔ DDL CHECK, space predicate ↔ `intents.ts`, and `job` member ↔ `JobListRow`**
   - What: a test pinning `watch.schema.json`'s `audit.eventType` enum **derived at runtime** from the `audit_events` CHECK (parsed from the migration source, not hardcoded) so a new DDL kind fails CI until the schema enum matches; asserting the low/high classification predicate resolves to `run.%` / non-`run.%` with the `DB_EVENT_SEQ_BASE` threshold consistent with `intents.ts`; **and a `job`-member drift assertion — the `job` event's *payload* field set (its schema keys **minus** the common NDJSON envelope keys `v`/`event`/`at`, which every event member carries per Task 3) must equal the key set of `@atlas/jobs`' `JobListRow` (the SSOT shape), so a later `JobListRow` field add/rename fails CI until the schema replica matches** (the same owner-to-replica guard the audit enum gets — the job schema is not left un-pinned). The replica is *exercised* (not just structurally compared) by a **real `job` event in the Phase 4 liveness test** (`watch.liveness.cli.test.ts`), **not** the Phase 6 `--once` sweep — `--once` emits only `watch.hello` and returns before any domain event, so it cannot carry a `job` line.
   - Files: new `tools/watch-eventtype-drift.test.ts`, `tools/watch-job-shape-drift.test.ts`.
   - Interfaces — **Consumes:** the CHECK in `0001_core.ts`, `DB_EVENT_SEQ_BASE` and the `nextDbEventSeq` predicate from `intents.ts`, `JobListRow` from `@atlas/jobs`.
   - Test: fails on a deliberate DDL-only addition **and** on a deliberate `JobListRow` field change; green when all sides match.
...
2. **Atomic attach: resolve probe, then synchronous consistent capture — capturing the immutable replay window inside the transaction**
   - What: open the ledger with `openReadonlyLedger(path)` (Phase 1 Task 4). Absence has **two** distinguishable shapes, both → a `DetachedLedger` (no connection, no cursor; `resume`/`replay` absent): (i) the file is missing → the opener throws the distinguishable missing-path error; (ii) the file exists but migrations have not completed → the open succeeds but `ledgerSchemaState(db) === "absent"` (Phase 1 Task 4) — the created-but-not-yet-migrated poll race. A schema-absent open is **not** allowed to proceed into snapshot/baseline queries (which would hit missing tables and become a fatal exit-4 fault); it is classified detached and re-polled. **Any unsuccessful attach (missing-path throw, schema-absent, or an aborted consistency retry) closes any connection it opened before returning** — no leaked read handles across the detached re-probe loop. Config missing/invalid → `config-invalid`, exit 2. When present, the probe and the SQLite snapshot are two observations that must describe the **same ledger moment**, or `deriveAnchorVerdict` compares a broker `head/count` from before a commit against a live `head/count` from after it and emits a **false `anchorOk:false`**. Coordinate them with a **bounded optimistic retry** (default 3 attempts, `ATLAS_WATCH_ATTACH_RETRIES`): capture the ledger identity via **`captureLedgerIdentity(conn)` from `@atlas/sqlite-store`** (Phase 1 Task 4 — it `fstat`s the connection's own descriptor inside the store package; the CLI never opens an fd itself), never a separate `stat(path)` — a `stat(path)` taken after the open could describe a *different* file if an atomic restore swaps the path during the probe awaits, binding the poller's identity check to the wrong inode; the fd-based identity always describes the exact bytes the connection reads — plus `PRAGMA data_version`, **resolve the broker probe asynchronously** — `const probe = await resolveAnchorProbe(broker, env)` (Phase 1 Task 1); if `probe.kind === "protocol-error"` this is fatal (Phase 5 Task 4 maps it to exit 4) — **then**, from **one brief synchronous read transaction on the connection that becomes the steady-state poller** (never a throwaway), re-read `data_version` **first** and, if it differs from the value read before the probe, **abort the transaction and retry** (the ledger moved under the probe); only when `data_version` is stable across the probe interval does the transaction capture a single consistent point: `deriveSnapshot({conn, anchorPath, env, probe})` (synchronous, no await inside the transaction), the four source baselines, the ledger identity, the initial `data_version`, **and — if `--since-seq` — the exact ordered replay rows read *inside this same transaction***. The replay window is therefore an **immutable snapshot of rows**, not a stored `(sinceSeq, upperSeq)` bound re-queried later: `ReplayWindow = { sinceSeq, rows }` where `rows` are the low-space (`run.%`) audit rows with `sinceSeq < seq`, in strict `seq` order, materialized now; `events = rows.length`. A row that commits into the range *after* the transaction closes cannot appear in `rows`, cannot inflate the announced `events` count, and is delivered later by the live diff (preserving the §7.4 pending-hole behavior). Then close only the transaction, keep the connection. Daemon reachability for the snapshot's `daemons` object is probed via `probeDaemon` **before** the transaction alongside the broker probe; the attach composes `WatchSnapshot = { ...deriveSnapshot(...), daemons }` and records the **full initial probe outcome** into `baselines.daemonState` — a clean probe stores `{known:true, reachable}` (the comparand for the first Phase-4 transition, no re-probe, no hidden state); an initial **`fault`** stores `{known:false}` (so it is **not** mis-recorded as `reachable:false`, which would fabricate a phantom `false→true` transition on the next success) and appends a `watch.error` to `baselines.pendingDaemonFaults`, which the orchestrator flushes **immediately after the hello** (§7.1). The `daemons` snapshot object shows `reachable:false` for a fault only as the point-in-time hello view; the transition baseline stays `{known:false}`. `emitHello` (declared below) is the sole reader of this composed snapshot + cursor.
   ...
   - Interfaces — **Produces:**
     ...
     // LedgerIdentity is defined + owned by @atlas/sqlite-store (Phase 1 Task 4) and captured via captureLedgerIdentity(conn) — device/inode from fstat on the OPEN fd (inside the store), never a stat(path). Imported here, not re-declared.
     ...
     interface SourceBaselines {
       ...
       jobsMap: Map<string, JobListRow>;    // the FULL projected row per jobId — diff on the whole projection, not a subset. A change confined to workflow/maxAttempts/lastError (lastError can come from joined attempt data without moving state/attempts/updatedAt) still triggers a job event; tracking a subset would silently drop it.
       backupRow: { watermarkSeq: number; healthy: boolean; lastBackupAt: string | null; updatedAt: string } | null;
       daemonState: { broker: DaemonBaseline; egress: DaemonBaseline };   // full initial probe outcome per daemon — the baseline the first transition compares against (no re-probe, no hidden state)
     ...
     // so a fault→success sequence does not fabricate a reachable:false→true transition (and emits its watch.error instead).
     type DaemonBaseline =
       | { known: true; reachable: boolean }        // a clean reachable/unreachable probe — the transition comparand
       | { known: false };                          // the initial probe faulted — no observed reachability yet
     ...
     // The exact public config payload the watch.hello `config` field serializes — the frozen view of the resolved options, defined once so emitHello needs no other context:
     ...
     interface AttachedLedger {
       attached: true;
       path: string;                         // the ledger path echoed in hello ledger.path — stored on the attachment, not global state
       config: WatchConfigView;              // the hello config payload — captured at attach
     ...
     interface DetachedLedger {
       ...
       daemonState: { broker: DaemonBaseline; egress: DaemonBaseline };   // last-known probe outcome — the detached loop compares each re-probe against it to emit daemon transitions / watch.error
       pendingDaemonFaults: WatchErrorLine[];   // initial detached-probe faults, flushed after the detached hello
     }
     // Hello emitter — the sole constructor of a watch.hello line from an Attachment. It reads ONLY the Attachment (path, config, snapshot, resumeCursor/replay) — no global or ambient state. Declared here (referenced by the Phase 3 Task 4 orchestrator) so hello is built from captured state, never a re-probe:
     ...
     ```
     Low-space predicate = `event_type LIKE 'run.%'` (`seq < DB_EVENT_SEQ_BASE`) throughout; high-space = the complement. **Consumes:** `openReadonlyLedger`, `captureLedgerIdentity`, `ledgerSchemaState` (all `@atlas/sqlite-store`), `resolveAnchorProbe`, `deriveSnapshot` (synchronous), `probeDaemon`. The seed rule (§9.1): scan the existing low-space (`run.%`) `seq` set to seed `auditContiguousPrefix` + baseline-seen marks; scan the high-space seqs to seed `highSpaceEmitted`; the pre-attach backlog is not replayed but a later gap-fill still emits.
   - Test (create): `apps/cli/test/watch.hello.cli.test.ts` (real child `dist/bin.js`) — `--once` emits exactly one `watch.hello` conforming to `watch.schema.json`, exit 0; snapshot equals `status --json`'s summary on shared keys (run `watch --once` **before** `status`; exclude `audit.headSeq`/`audit.head` — §13.2); ledger-absent hello reports `attached:false` with `resume`/`replay` absent. **Immutable-window unit test** (in-process): open an attach with `--since-seq k`, then commit a new low-space row into the `> k` range from a second connection *after* attach returns; assert `attachment.replay.rows` still has the pre-commit count and does not contain the late row (it will surface via the live diff instead). **emitHello self-sufficiency unit test** (in-process): construct an `AttachedLedger` and a `DetachedLedger` directly, call `emitHello` with a capturing `emit`, and assert each emitted line validates against `watch.schema.json` and carries `ledger.path` (from `att.path`) and `config` (from `att.config`) — proving the emitter needs no context beyond the Attachment.
3. **Poll loop reporting completion + inode/schema guard (async, serialized ticks)**
   - What: every `pollMs`, read `PRAGMA data_version` on the transaction-free poller connection (never hold an open txn across ticks — it pins the value). **Read `data_version` exactly once per tick and store that observed value *before* invoking `onTick`; never re-read or refresh it after the callback returns.** Refreshing after the diff would strand a commit that lands *after* the callback's source read but *before* the refresh: the new version would be cached without its rows ever being diffed, and the event would stay invisible until some unrelated later commit — a lost wakeup. Storing the pre-callback value means such a commit leaves `data_version` ahead of the stored value, so the next tick still fires. On change (or an `attached` transition) invoke `onTick` — an **async** callback that is **serialized** (a tick never starts until the prior tick's promise resolves), so emission can await pipe backpressure (Phase 4) without overlapping or reordering. Each tick also `stat()`s the ledger path and compares `(device, inode)` + schema head vs `AttachedLedger.identity`; mismatch/vanished → the loop resolves its completion promise with `"reattach"` (the detach/re-attach *action* is Phase 5 Task 2; Phase 3 only detects and surfaces it). A `stop()` call (signal handler) resolves the promise with `"stopped"`. `fs.watch` on `-wal` is an **optional** wake hint — **ship poll-only in v1**.
   - **The loop returns a completion channel** so the `runWatch` orchestrator (Task 4) can await the outcome and decide re-attach vs exit — closing the finding that a bare `{ stop() }` handle stranded the `"reattach"` result:
     ...
     function runPollLoop(
       ...
       onHeartbeat: () => Promise<void>,   // fired every heartbeatSeconds of QUIET (no data_version change) AND coalesced-in after activity — the loop owns the heartbeat cadence, since a stable ledger never triggers onTick
     ): PollHandle;
     ```
     `done` resolves `"reattach"` when a tick (or `onTick`) reports the inode/schema change, `"stopped"` when `stop()` is called; the loop stops ticking once resolved. The `onHeartbeat` callback runs on the **same serialized queue** as `onTick` (never concurrent with it), so quiet-period heartbeats and daemon probes (Phase 4 Task 3) are emitted in order even when the database is idle. **Consumes:** `attachLedger`.
   - Files: new `apps/cli/src/watch/poll-loop.ts`.
   - Test (add to) `apps/cli/test/watch.hello.cli.test.ts`: **in-process callback test** — a change committed by the test process fires `onTick` within 2×`pollMs`; two rapid commits produce non-overlapping serialized invocations; an inode swap resolves `done` with `"reattach"`; a `stop()` resolves `done` with `"stopped"`; **lost-wakeup guard — block inside an active `onTick`, commit a new row after that callback's source read, release it, and assert a subsequent tick still fires for the late commit** (proving `data_version` is stored pre-callback, never refreshed after).
4. **`runWatch` orchestrator: attached + detached loops, consuming each loop's completion**
   - What: a single top-level orchestrator that owns the whole lifecycle across both ledger states and is the sole caller of `runPollLoop`, `runDetachedLoop`, `reattach` (Phase 5), and the emitters. It resolves the finding that nothing polled an initial `DetachedLedger`: an **explicit detached re-probe loop** re-runs `attachLedger` every `pollMs` until the ledger appears, emitting detached heartbeats at the heartbeat cadence meanwhile **and — at that same cadence — re-probing the daemons: a reachability transition against `DetachedLedger.daemonState` emits a `daemon` event, and a probe `fault` emits a non-fatal `watch.error(source:"broker"|"egress")` (the detached loop is not heartbeat-only; daemon health is observable while the ledger is gone)**, then hands a freshly-`AttachedLedger` back so the orchestrator emits a fresh `watch.hello` (and, if `--since-seq` was pending, runs the deferred replay against that first attached ledger — Phase 5 Task 1). Control flow (Phase 3 wires attached↔detached transitions and exit; the re-attach *action* body and replay land in Phase 5):
     ```ts
     interface DetachedHandle { stop(): void; done: Promise<"stopped" | { attached: AttachedLedger }> }
     // Single serialized queue, exactly like runPollLoop: an attach attempt and a heartbeat/daemon-probe emission never overlap, and the NEXT attach attempt does not start until the prior attempt's promise resolves. With a 500 ms poll and a 2 s probe this prevents several in-flight attaches from racing to hand back two fresh AttachedLedgers. A stop() drains the in-flight emission then resolves "stopped".
     // Takes the DetachedLedger (carries path/config + the daemonState baseline it mutates across re-probes) and the GENERAL line emitter — so the loop emits detached heartbeats, daemon transitions, AND watch.error faults, not heartbeats alone.
     function runDetachedLoop(att: DetachedLedger, opts: WatchOpts, ctx: AttachContext, emit: (line: unknown) => Promise<void>): DetachedHandle;
     ...
     async function runWatch(path, opts, ctx, emit): Promise<number> {   // returns the process exit code
       ...
       await flushPendingDaemonFaults(att, emit);                        // initial-attach probe faults → watch.error, right after hello
       for (;;) {
         if (att.attached) {
           if (att.replay) { await runReplay(att.replay, emit); await emitPostReplayHeartbeat(att, emit); }  // Phase 5
           const h = runPollLoop(att, opts, onTick, () => heartbeatTick(att, emit));  // onHeartbeat drives quiet-period heartbeats + daemon probes
           ...
           const d = runDetachedLoop(att, opts, ctx, emit);   // att: DetachedLedger — loop emits heartbeats + daemon transitions + watch.error via the general emitter
     ...
     - **Deferred symbols land as typed stubs in Phase 3 so the build gate passes (no forward reference to unwritten code).** `runReplay`, `emitPostReplayHeartbeat`, `emitWatchError`, `reattach`, `diffSources`, **and `heartbeatTick`** (produced in Phase 4 Task 3, but the Phase 3 orchestrator's `onHeartbeat` closure calls it) are **declared and exported with their final signatures in Phase 3**, with a minimal no-op/typed-placeholder body (e.g. `reattach` = re-run `attachLedger`; `runReplay`/`emitWatchError` = typed no-ops returning immediately; `diffSources` = returns `[]`; `heartbeatTick` = emits only the bare `watch.heartbeat` cursor line in Phase 3, daemon-probe/`watch.error` branches filled in Phase 4). Phase 4/5 **fill the bodies**, never change the signatures. So `pnpm --filter @atlas/cli build` and the Phase 3 `tsc` gate pass with no unresolved imports, and Phase 3's own tests (`--once`, hello, in-process tick, startup-detached) exercise only the paths whose bodies exist. This is why the registry row **stays `implemented:false` until Phase 5**: the handler is *dispatchable* (for tests) but its replay/re-attach/domain-event semantics are stubs until their phase lands, so a user is never handed a half-working `watch` as a shipped command — the flip is the completeness gate.
...
1. **Per-source diff cursors**
   - What: on each pending-diff tick, diff the four sources per §9.1:
     ...
     - `jobs`: full-table diff via **`listAllJobs`** (Phase 1 Task 2, `@atlas/jobs` — the unpaginated single-transaction reader, **not** the 500-capped `listJobs`) vs `jobsMap`, comparing the **entire projected `JobListRow`** (a deep-equality/field-wise compare, not just `state`/`updatedAt`) so a change isolated to `workflow`/`maxAttempts`/`lastError` still emits — the SSOT reader, never a hand-rolled query, so every job at the §4 scale is observed regardless of count.
...
3. **Control events: heartbeat + non-fatal error + ordering/coalescing**
   ...
   - Test (create): `apps/cli/test/watch.ordering.cli.test.ts` — two rapid job transitions in one tick → one event, final state (§13.5); heartbeat carries the prefix cursor; **daemon transition events on socket toggle** (reachable→unreachable→reachable, the §13.3 companion — `probeDaemon` exists, no detach needed); **a probe `fault` outcome emits `watch.error` (`source:"broker"`) and the stream keeps streaming** (the Phase 1 Task 3 fault seam proven end-to-end); **initial-attach fault, then success — no phantom transition:** an initial probe `fault` emits a `watch.error` right after hello and records `daemonState:{known:false}`, so a subsequent successful probe emits **no** `daemon reachable:false→true` transition (the `{known:false}` baseline is not a `reachable:false` comparand) — only a first real transition once a clean reachability is observed. **The recoverable-ledger-fault assertion (`watch.error(source:"ledger")` then continued streaming) is *not* here** — it requires the detach/re-attach path (Phase 5 Task 2) and is asserted in `watch.reattach.cli.test.ts` (Phase 5).
...
4. **Exit codes + signal/EPIPE handling + fatal protocol-error mapping**
   - What: per §10.1 — `--once` done / SIGINT / SIGTERM / EPIPE ⇒ flush the current line, exit **0**; config/vault startup fault ⇒ exit **2** envelope; internal fault **or a broker `protocol-error` probe result during the snapshot** (Phase 1 Task 1's `AnchorProbe.kind === "protocol-error"`, i.e. a thrown `BrokerRefusal("broker.bad_request")` — *not* `unreachable`, which is data and degrades to `sqlite-only`) ⇒ exit **4**, one final `error-envelope` line, no event line after, no raw stack; usage ⇒ exit **5**. EPIPE exits 0 quietly (never 141); other signals keep default 128+n. **A single persistent shutdown latch, installed once at startup — signals are never lost between loop handles.** The SIGINT/SIGTERM handlers are registered **once** (not per-loop) and set a `stopRequested` flag **and** call `stop()` on the loop handle *if one is currently active*. `runWatch` checks `stopRequested` at **every await boundary** — before attach, after hello, before/after replay, before re-attach, and between loop instances — and short-circuits to a clean return 0 if set. So a signal arriving during the initial probe, hello emission, replay, post-replay heartbeat, or the gap between loops is **remembered** and honored at the next boundary, not swallowed. On stop, the currently in-flight emit (already serialized) is awaited to completion — the final line is **drained**, not truncated — before exit. The attach path (Phase 3 Task 2) already resolves the probe before the transaction; here that resolved `kind` is mapped to the fatal path.
   ...
   - Interfaces — **Produces:** signal handlers wired to the active loop's `stop()`; `emitFatalEnvelope(err: unknown): Promise<never>` — **async** so it can `await emitLineAwaitable(...)` (the awaitable writer that honors backpressure/`drain`) for the final envelope **before** calling `process.exit(code)`; typed `Promise<never>` (the awaited body ends in `process.exit`, never returning), which strict TS accepts where a synchronous `: never` could not await the write. Under backpressure the final envelope is therefore guaranteed flushed, not dropped on an early exit.
...
1. **Runtime-derived inventory + sweep test + per-command invocation adapter map**
   ...
   - Interfaces — **Consumes:** registry rows + schemas (derive inventory), the draft-2020-12 validator already in `@atlas/cli-contract-tools`, `withFixtureVault` + the in-process broker/egress harness (via the new `@atlas/testing` devDep), the built `dist/bin.js`, the existing graduation/quarantine test harnesses (reuse, don't rebuild), the fixture authorization signer.
   - Test: §13.8 — the sweep passes for all runtime-inventoried commands, each under its adapter; `watch` doesn't hang the sweep. It runs under `pnpm --filter @atlas/cli-contract-tools test`.
...
- **`watch` ↔ `@atlas/jobs`:** the `job` event and `jobs list` share one owner — `packages/jobs/src/repo.ts`'s `listJobs`/`projectJobListRow`/`JobListRow` (with `updatedAt`). `watch` calls `listAllJobs` per tick; it never issues its own jobs SQL.
...
**Live drive (manual, per test-live posture):** on this Mac against the real provisioned install — `brain watch --json | head` shows hello + real events while `brain enrich --apply` runs in another terminal (`enrich` confirmed present in the registry); kill/re-attach with the printed cursor. Documented as a PR runbook step (§13.9), not automated. **This is a liveness demo, not the read-only proof — and it mutates the vault via `enrich`, not `watch`.** `watch`'s read-only guarantee is proven by the **automated** `watch.readonly.cli.test.ts` (§13.10, Phase 5 Task 5), which snapshots audit head / watermark / lock state and asserts nothing moved. The mutations the live drive observes come from `enrich --apply` (that is the event source being watched); run the live drive against a **scratch/disposable vault**, since re-running it is not idempotent (`enrich` creates jobs/audit rows/model calls). The no-rollback rationale covers `watch`'s own effects (none); it does not license `enrich`'s.
