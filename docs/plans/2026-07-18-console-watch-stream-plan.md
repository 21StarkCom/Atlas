...
4. **Read-only ledger opener in `@atlas/sqlite-store`**
   ...
   - Interfaces — **Produces (from `@atlas/sqlite-store`):** `function openReadonlyLedger(path: string): SqliteDatabase`; `function ledgerSchemaState(db: SqliteDatabase): "ready" | "absent"`; re-export of the `SqliteDatabase` type.
