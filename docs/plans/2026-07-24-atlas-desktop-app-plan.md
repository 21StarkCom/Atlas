...
|---|---|---|---|---|
| 1 | Scaffold + readiness indicator (capability 1) | `apps/desktop` workspace, engine session, CLI-spawn wrapper, health/`CHECK_SEVERITY`, poller, menubar glyph + popover scoreboard; ADR-0004; `@atlas/models` presence probe | suite green both legs; **no native dep**; `contract:check` unchanged | L |
...
| 6 | Packaging + live drive | `.app` bundle (`brain` + `ATLAS_ROOT` resolution), mandatory live drive on the real Mac | packaged `.app` resolves the bundled CLI; live-drive evidence recorded (human-run) | M |

## 2. Prerequisites

- Node ≥ 24, pnpm **11.15.0** (the pin gotcha — a broken 11.12.0 exited 127/1). CI is Node 26.
- The reviewed spec + ADR-0004 are on `main` (spec merged in #355). This plan lands next.
...
**Parallelizable:** the ADR + the plan land together in Phase 1's PR. Phase 5 (a11y) drafting can begin against the spec any time but merges after the surfaces it covers exist (Phases 1–4).
...
**Delivers.** The `apps/desktop` workspace and the entire readiness path, testable without launching Electron; ADR-0004 + this plan on `main`; the first `@atlas/models` export.
...
2. **ADR-0004 + plan land** in this PR.
...
4. **Health + severity (`src/main/health.ts`).** `CHECK_SEVERITY` map; one exported function producing a `SessionSnapshot` from `checks[]` + `configLoaded`. Table-driven tests (every health-table row, incl. `checks:[]`→`cli-unreachable`, unknown-check→fail, config-precedence, fail-dominates-warn).
5. **`@atlas/models` presence probe export.** A presence-only wrapper returning `CredentialStatus` (`present` + `source ∈ {env,keychain,none,unknown}`) — env else `security find-generic-password -s <service>` (no `-w`, no `-a`); item-not-found ⇒ `none`, other failure ⇒ `unknown`. Internal refactor, no CLI-surface change.
6. **Engine session + poller (`src/main/session.ts`).** Init sequence (load config → probe key presence → immediate poll → arm interval); single-flight poll; `lastCheckedAt` on completed polls only; the generation counter. (Restart, cancellation, coherence, reconciliation are Phase 2 — here just the forward path.)
7. **Menubar indicator (`src/main/tray.ts` + `src/renderer/`).** Glyph (green/amber/red) from the snapshot; popover scoreboard rendering the five checks + `lastCheckedAt`; on-demand refresh on popover open. Renderer renders from the delivered snapshot and **recomputes no health/severity**.
...
4. **Config-generation coherence.** Snapshots carry `configGeneration`; before each poll compare the on-disk config fingerprint and Restart on drift rather than mixing generations.
...
**Gate.** Config round-trip (incl. comment/order/unmanaged preservation, note_globs clear-omits-key, no-op, atomicity, pre-write re-read under concurrent edit) + pin + credential (empty-key, account-unresolved, three-valued probe, no-secret-in-argv/logs/IPC) + IPC-validation tests green. **Resolves spec open-questions:** the `setGeminiApiKey` + `canonicalizeVaultPath` extractions.
...
**Tasks.**
1. **Packaging + resolution.** Bundle `apps/cli/dist/` + the `docs/specs/cli-contract/` tree into the `.app` `Resources`; spawn the bundled `brain`; point `ATLAS_ROOT` at the bundled tree (dev resolves the sibling repo build). Absent resources ⇒ `cli-unreachable` + reinstall hint. (Resolves spec open-question #2: packaging topology mechanics.)
