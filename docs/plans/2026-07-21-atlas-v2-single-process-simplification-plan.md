...
```bash
git rev-parse 'v1-fortress^{commit}'                        # annotated tag peels to the commit
[ "$(git rev-parse 'v1-fortress^{commit}')" = "$(git rev-parse origin/main)" ] && echo "tag == pre-demolition main"
...
6. **`link` — new command (row + fixture + 7-field schema + handler).**
   ...
   - **`link.schema.json` is a 7-field object**: the exact contract is `{ "type":"object", "additionalProperties":false, "required":["action","source","target","predicate","alias","commit","noop"], "properties": { "action": {"enum":["added","updated","removed","related","noop"]}, "source":{"type":"string"}, "target":{"type":"string"}, "predicate":{"type":["string","null"]}, "alias":{"type":["string","null"]}, "commit":{"type":["string","null"]}, "noop":{"type":"boolean"} } }`. `commit` is `null` **iff** `noop:true`.
...
2. **`deprovision-macos.sh` + the allowlist companion gate (exact enumeration).**
   - What: author `provisioning/deprovision-macos.sh` (`set -euo pipefail`, gated on explicit `--confirm`, `sudo`, **never CI**) whose deletion set is an **exact, exhaustive allowlist** — an engineer can implement + verify exact-match deletion from this list. **Every deletion is idempotent — an already-absent resource is a success, not a failure (guard each `launchctl bootout` / `sysadminctl -deleteUser` / `dscl -delete` / `security delete-generic-password` behind a presence check) — so an interrupted run reruns cleanly to completion; only an unexpected failure (e.g. a nonempty parent under `rmdir`) stays fatal.** The allowlist enumerates precisely:
...
- **Any retired subsystem:** check out the `v1-fortress` tag (Phase 1) — the sole revival path, by design. The tag revives **code + provisioning only, not migrated data** — Phase 5's destructive `db migrate` (0014/0015) discards v1 claims/evidence/provenance/cursor/ledger rows unrecoverably, so the pre-migrate SQLite file copy (Phase 5 task 3) is the only data undo.
...
- **The two irreversible steps — Phase 5's destructive live `db migrate` (v1 data discard; the pre-migrate SQLite file copy is the only undo) and the live-Mac deprovision** — are human-run, `sudo`-gated on explicit `--confirm`, never CI; deprovision's "undo" is re-provisioning from the tag, and the migrate's go/no-go is the green task-3 drive, both accepted deliberately.
