...
| `--once` | off | — | Emit `watch.hello` (the full snapshot) and exit 0. The cheap probe/test seam; also what SP-2 calls at attach before deciding a resume point. |
| `--poll-ms <n>` | 500 | 100–10000 | Ledger change-detection cadence (§9). |
| `--heartbeat-seconds <n>` | 30 | 5–300 | `watch.heartbeat` emission + daemon reachability probe interval (§7.4, §9.3). |
...
|---|---|---|
| `watch.hello` | first line; and again after a mid-stream ledger re-attach (§8.3) | `pid`, `ledger: {attached, path}`, `snapshot` (§7.2), `resume: {auditHeadSeq, earliestAvailableSeq}`, `replay: {sinceSeq, events} \| null` (present iff `--since-seq`), `config: {pollMs, heartbeatSeconds}` |
| `watch.heartbeat` | every `heartbeatSeconds` of quiet **or** activity | `resume: {auditHeadSeq}` — the k8s-BOOKMARK / SSE-keep-alive dual: liveness signal *and* cursor checkpoint. |
...
| `backup` | `backup_watermark` (single row) | any column change | `watermarkSeq`, `healthy`, `lastBackupAt?`, `updatedAt` — the DDL columns camelCased. (`coveredSeq` appears only in the snapshot, where it is derived by the shared `status` code; the event mirrors the row.) |
...
- **Per-source ordering:** `audit` events are emitted in strict `seq` order; `model_call` in `(created_at, call_id)` order; `job` events in observation order per job.
...
- **At-least-once across restarts:** a consumer resuming with `--since-seq` may re-receive audit events at the boundary; `seq` is the idempotency key. Within one process the stream is exactly-once per row.
...
`audit_events.seq` is a global monotonic allocation — a real cursor. **Replay is defined for the `audit` stream only:** `--since-seq <n>` re-emits durable audit rows with `seq > n` (exclusive) inside `watch.hello`'s `replay` window, then the live stream continues from the head.
...
`model_calls` rows are durable and could support replay by `(created_at, call_id)`; v1 deliberately omits it — the Console's model-call history view reads the ledger through a paginated read command instead (recommendation R2, §14). Cost: an unwatched window's model calls appear only in history views, not as live events. Accepted.
...
1. Every `pollMs`, read **`PRAGMA data_version`** — its value changes when *any other connection, including connections in separate processes,* commits a change to the database file, and never for our own connection (sqlite.org pragma docs, verbatim scope; see appendix §16.7). In WAL mode the check rides the wal-index in the `-shm` shared memory — no table I/O, no write lock, safe at 2 Hz forever. Two rules make it correct: the polling connection **never holds an open transaction** (inside a read transaction the value is pinned to the snapshot), and values are only compared within one connection's lifetime (they are connection-local; a reconnect re-baselines).
2. On `data_version` change (or a `ledger.attached` transition), diff the four sources: `audit_events` by `seq > cursor`; `model_calls` by `(created_at, call_id) > cursor`; `jobs` by `updated_at >= lastSweep` re-read + in-memory `jobId → (state, attempts, nextRunAt, updatedAt)` map compare; `backup_watermark` single-row compare.
3. Emit events per §7.4 ordering; advance cursors; update the heartbeat's `resume`.
...
2. **Implementation PR(s):** handler + `registerCommand` barrel import, flip `implemented: true`. `command-registration.test.ts` (the #145 guard) enforces the flip-with-handler pairing; `checkImplementedSchemas` enforces schema existence; the §7.5-bijection gates enforce the fixture row.
...
No exit 7 (nominal provider-retryable — `watch` never talks to a provider), no exit 1/3/6 paths exist. Signal handling is explicit: `SIGINT`/`SIGTERM` flush the current line and exit 0; `EPIPE` on stdout exits 0 quietly (never 141); any other signal keeps default 128+n semantics. The single-error-envelope rule holds (§6) — `watch` is not a batch exception.
...
- **Inventory (acceptance artifact):** the audit enumerates every registry row whose schema declares `executionClass: "read"` or `"audited-read"` — currently 22 commands: `db status`, `db verify`, `doctor`, `evidence review`, `git status`, `git review`, `git verify`†, `graduation audit`, `graduation scan`†, `index eval`†, `index status`, `index verify`, `inspect`, `jobs list`, `note history`, `note related`, `note show`, `quarantine inspect`‡, `source list`, `source show`, `source trust show`, `status` — plus `watch` itself (via its per-line union). († = read-classed but capable of repair/egress side effects per its own schema; the audit validates output shape only. ‡ = privileged; exercised with the test-mode authorization fixtures.)
- **The gate:** one new `apps/cli/test/json-conformance.sweep.test.ts` that drives each inventoried command `--json` against the standard fixture vault (`withFixtureVault` + the in-process broker harness) and validates stdout against the command's `schemaRef` with a draft-2020-12 validator (the dependency already exists for `cli-schemas.test.ts`). A command whose live output violates its schema fails CI from then on.
...
- **Terminal-injection stance:** `--json` output is exempt from `render/safe.ts` sanitization by existing convention (`emitJson` writes raw JSON; JSON string escaping prevents the CSI/OSC class at the encoding layer as long as emitted strings are `JSON.stringify`-produced, which the envelope guarantees). `watch` introduces no free-text fields sourced from vault content — the riskiest fields (`workflow`, `lastError`, `eventType`) are enum-constrained or writer-controlled identifiers.
...
2. **Hello & once:** `--once` emits exactly one `watch.hello` conforming to the schema and exits 0; snapshot equals `status --json`'s summary for the same fixture state (field-for-field on the shared keys) — the §7.6 SSOT assertion.
3. **Liveness:** with `watch` attached, enqueue a job / finalize an attempt / append an audit event / flip the watermark via the harness; assert the corresponding event arrives within 2×`pollMs`, correctly shaped, in per-source order. This is the §9.4 cross-process `data_version` pin (writer = the test process, watcher = the spawned child).
...
5. **Coalescing & idempotency:** two rapid job transitions inside one poll tick produce one event with the final state; killing and resuming with the last heartbeat's cursor re-delivers no gaps (and only boundary duplicates).
...
8. **Conformance sweep (§11):** the new sweep test passes for all inventoried commands, with any handler/schema fixes landed in-chain.
...
| R2 | Should `model_call` replay exist (`--since-seq` is audit-only)? | **No.** Add a paginated `model_calls` read surface later if the Console's history view needs it (a natural `jobs list`-shaped command); replay-on-attach for cost rows the snapshot already summarizes is redundant transport. |
