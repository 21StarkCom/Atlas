...
Let `E` = `earliestAvailableSeq`, the lowest retained `seq`. Replay is exact (no rows pruned out from under the cursor) as long as `--since-seq n` satisfies `n ≥ E − 1` — because `seq > n` (exclusive) then starts at or above `E`, so rows `n+1..` are all still present. A cursor is genuinely **too old** only when `n + 1 < E`: rows `n+1..E−1` have been pruned (possible after `db restore` to an older cut, or a future retention policy) and cannot be replayed. In neither case does `watch` fail: `watch.hello.resume.earliestAvailableSeq` exposes `E`, `replay.events` counts what was actually re-sent, and the consumer decides whether to re-baseline. **Too-old is detected by comparing `n` against the reported floor (`n + 1 < E`) — never from a gap between `n` and the first replayed `seq`.** Such a gap is not proof of pruning: a lower `seq` may simply be a not-yet-committed intent that §7.4 will still deliver when it lands (cursor 10, row 12 committed, row 11 pending ⇒ replay opens at 12, yet 11 is not lost). Kubernetes forces a re-list on 410; here the snapshot *is* the re-list and it is always sent.
...
4. **Replay & its checkpoint:** seed N audit rows, run `--since-seq k`, assert exactly rows `k+1..N` replay in strict `seq` order and that `watch.hello.replay.events` equals the replayed line count. Assert the **pre-replay checkpoint rule** (§7.4/§8.1): `watch.hello.resume.auditHeadSeq = k` (not N) during replay, followed by an immediate `watch.heartbeat` carrying `resume.auditHeadSeq = N`. **Crash-mid-replay:** kill the consumer after `watch.hello` but before the post-replay heartbeat, restart from the last *persisted* cursor, and assert no row `k+1..N` is skipped (the persisted cursor was still `k`). `--since-seq` more than one below the earliest retained row reports the floor and the actual re-sent count per §8.2.
