...
| `audit` | `audit_events` (insert-only, global monotonic `seq`) | new row | `seq`, `runId`, `eventType`, `gitHead?`, `createdAt`. `eventType`'s allowlist is **owned by the `audit_events` DDL CHECK** (the 9 `run.*` + 3 `db.*` kinds); `watch.schema.json`'s enum is a **replica the harness pins against drift** (§13.1) — never a second source of truth. `payload_hash` is deliberately omitted (an integrity internal, not a console signal). |
...
`watch.hello.resume.earliestAvailableSeq` (`E`) is the store's **retention floor** — the lowest `seq` the ledger still retains — moved only by a `db restore` to an older cut or a future retention policy, and read **from that pruning boundary, not computed as `MIN(committed seq)`** (which a pending low-`seq` intent could transiently overstate, §7.4). `watch` **never infers pruning from a gap in committed seqs**: a hole below the committed maximum may be a not-yet-committed intent that §7.4 delivers live when it lands (cursor 10, row 12 committed, row 11 pending ⇒ replay opens at 12, yet 11 is streamed later, not pruned). Pruning is only ever the *low-prefix* removal a restore/retention cut performs, and `E` names its boundary.

When `--since-seq n` is below `E` (`n + 1 < E`), rows `n+1..E−1` are genuinely gone and cannot be replayed — but `watch` does not fail: it exposes `E`, sets `replay.events` to what was actually re-sent, and **always sends the snapshot** (the re-list, exactly as Kubernetes forces on 410). Re-baselining from that snapshot is **lossless**: current state comes from the snapshot, and any still-pending low `seq` is delivered by the live stream (§7.4). Too-old is therefore an **advisory** signal the consumer MAY re-baseline on — never a precondition for correctness, and never derived from committed-seq arithmetic.
...
4. **Replay & its checkpoint:** seed N audit rows, run `--since-seq k`, assert exactly rows `k+1..N` replay in strict `seq` order and that `watch.hello.replay.events` equals the replayed line count. Assert the **pre-replay checkpoint rule** (§7.4/§8.1): `watch.hello.resume.auditHeadSeq = k` (not N) during replay, followed by an immediate `watch.heartbeat` carrying `resume.auditHeadSeq = N`. **Crash-mid-replay:** kill the consumer after `watch.hello` but before the post-replay heartbeat, restart from the last *persisted* cursor, and assert no row `k+1..N` is skipped (the persisted cursor was still `k`). `--since-seq` more than one below the earliest retained row reports the floor and the actual re-sent count per §8.2. **Pending-hole vs pruning:** with row 12 committed and row 11 a pending intent (§7.4), assert `--since-seq 10` opens replay at 12 **without** flagging too-old (the floor `E` is unmoved), and that row 11 is emitted live once it commits — a committed-seq gap is never classified as pruning.
