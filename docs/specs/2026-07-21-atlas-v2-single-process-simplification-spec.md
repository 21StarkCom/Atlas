...
**Moves/renames (binding).** Notes are matched by **stable note id** (frontmatter `id`), never by path. A file whose id matches an existing projection row but whose **path** differs — content hash unchanged — is a **move**: the projection row's `path` is updated in place, the move is counted in `changedCount`, and nothing is re-embedded (chunks/links/evidence key on the note id, not the path). A moved-and-edited note is simply a changed note. A new path carrying a **new** id is `new` + the old row ages out as `dropped` — the literal drop-recreate never fires for a pure move.
...
| `sync` | object | `{ pendingChangedCount: integer, pendingNewCount: integer, pendingDroppedCount: integer }` — the **complete** pending-reconciliation picture: hash-mismatched notes, working-tree notes with no projection row, and projection rows with no backing file. All three come from the one reconciliation routine (§ssot); they are **informational** (pending sync work is the normal state of a dirty tree, not unhealth) and do not drive `checks[]` |
...
Contract deltas from v1: `status.sync` **drops `lastIndexedHead`**, **adds the three pending counts**; the v1 `doctor` backup-health and signer-registry checks are **retired with their machinery** (§security) and do not appear in `checks[]`.
...
- **Operational state** (SQLite is the SoR, **not** vault-derived, retained across `db rebuild`): `jobs` / `job_attempts` (owned by `@atlas/jobs`), `source` registry (below), evidence state (below), `model_calls` (provider-call log for token/cost observability).
...
- **`sync` runtime contract — all four states, schema-validated** — run `sync --json` for (1) unchanged, (2) changed, (3) **newly added file** (`newCount:1`, indexed + retrievable), (4) deleted file; validate **every** payload against `sync.schema.json` (all five fields present, retired `head` absent), assert exact counts + `scannedCount`, stdout is clean JSON with logs on stderr only, and the second identical run is `noop:true` with no index write.
- **Dropped-note cross-store purge** — index a note, delete its file, `sync`; assert the projection row is gone, its `chunk` metadata rows are gone, its LanceDB vectors are deleted, and **`query` can no longer return it** (the retrieval-side orphan filter is exercised by interrupting a purge mid-way in a failpoint variant).
- **Move/rename** — `git mv` a note (same id, same content), `sync`; assert the projection row's `path` updated in place, `changedCount:1`, no re-embed (chunk rows untouched), links/evidence still resolve by note id.
- **Evidence v1→v2 cutover** — seed a database with the v1 evidence table shape; run `db migrate`; assert the exact ten-column v2 schema, the v1 audit-coupling columns/tables gone, and (accepted) the v1 rows discarded.
