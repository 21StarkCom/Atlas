...
| Jobs | List view whose membership + `createdAt` order come from `jobs list --json`; coalesced `job` events update the live state of already-listed jobs. A `job` event for an unseen `jobId` triggers a `jobs list` re-read — never a synthetic insert (the event carries no `createdAt`). |
...
- **Baseline-seeded** — `jobs{queued,failed}`: the snapshot's aggregate counts carry no per-job identity, so a coalesced `job` event (latest state only, no previous state) cannot be reconciled against them. The Console instead seeds a **per-job state map** from `jobs list --json` at attach/re-baseline and recomputes `queued`/`failed` by counting map entries, updating the map on each `job` event (latest state per `jobId`). The map holds each job's current state directly, so no previous-state or transition data is needed.
...
| 4 | **Coalescing + job-count map** — repeated `job`/`backup` for one id collapse to latest; two `audit`/`model_call` never collapse; `jobs{queued,failed}` recomputed from the per-job state map (seeded from `jobs list --json` at baseline), correct for a first-seen `jobId` and after coalesced multi-transition ticks. | Timeline losing events, a stale job state sticking, or job counts drifting from the unreconcilable aggregate snapshot. |
| 5 | **Resume/replay** — cursor selection per `resumeMode`, pre-replay `min(n,prefix)` not persisted until the checkpoint heartbeat, `--since-seq` exclusivity; a replayed `run.start`/terminal already in `snapshot.openRuns` does **not** re-apply (replay-phase rows touch timeline+cursor only, never `openRuns`). | Persisting an unsafe cursor mid-replay ⇒ a gap on next resume; replay double-counted into the snapshot. |
...
9. **SP-2 tracking issue.** No issue exists yet — open one at planning start (open repo issues are #60, #65 only; neither blocks SP-2). *Owner:* SP-2 lead.9. **Accepted contract gap (write-loop residual):** `test-plan` — underspecified: No concrete test proves that every command newly marked authorization-required in a registry fixture is discovered and routed through the signer flow, so the stated full authorizable-op-set behavior could regress silently. *Owner:* SP-2 implementer — add a registry-fixture test (a fixture `commands.json` flipping a shared row to `privileged`) asserting the Console's authorizable-op discovery picks it up and routes it through the signer flow.
