...
6. **Egress-gated action:** run one explicit `query` from the Console. Expected: result renders; no `query` on any timer; the query string appears in **no** Console log line (spot-check `log show --predicate 'subsystem == "com.atlas.console"' --last 5m`).
