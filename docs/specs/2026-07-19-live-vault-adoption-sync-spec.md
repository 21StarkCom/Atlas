...
5. **Upstream history rewrite — the non-ancestral cursor (ACCEPTED GAP, blocking at plan time).** The whole delta model assumes `refs/heads/main` stays a **descendant** of `last_absorbed_oid`, and that assumption is never stated or defended — despite `refs/heads/main` having three uncoordinated writers (brain-hub sync, Obsidian, direct GitHub pushes), any of which can rebase or force-push. Nothing in `interfaces`, `behavior`, or `test-plan` covers it. Specifically unresolved:
   - **`interfaces`:** no result or error contract for an upstream rewind, a force-push, or a `last_absorbed_oid` that no longer resolves (unreachable after a `git gc`) or is no longer an ancestor of the upstream head. There is no error code for it and no `sync status` field that surfaces it.
   - **`behavior`:** `git diff <cursor>..refs/heads/main`, `behindBy`, commit ordering, and continuation are all **undefined** when history diverges. `behindBy` in particular is computed as a count between two commits that may have no ancestry relation at all.
   - **`test-plan`:** no test force-pushes upstream, rewinds it, or makes it non-descendant of the cursor, so none of the above is proven either way.
   - **The policy decision itself is open:** on detecting a non-ancestral upstream, does sync **reject** (halt, exit non-zero, require operator action), **reset** (re-baseline the cursor to the new head and accept the gap in absorbed history), or **reconcile** (diff against the merge-base and absorb the symmetric difference)? Each has a different correctness story for `refs/atlas/main`, whose history would otherwise silently diverge from the upstream it claims to mirror.

   Owner: operator + plan. This surfaced in the spec's round-2 contract verification and was **accepted rather than resolved**, per the 2-round review cap — it is the first thing the implementation plan must settle, before any sync code is written.
6. **Deferral of 60-C / 60-D / 60-E (decision recorded).** This spec bounds itself to the adoption-and-sync core (60-A + 60-B); 60-C (purge E2E, depends on #54), 60-D (`scale-bench` + CI regression subset — the at-scale O(delta) validation), and 60-E (Tier-2/3 workflow runs + rollback under the production authorizer) are **peeled into follow-on issues**, each only loosely coupled to the sync core and each substantial enough to blow the 2-round review cap if bundled. Open: this spec's plan must **sequence** those follow-on issues after 60-A + 60-B land (60-D in particular closes the at-scale half of the O(delta) success criterion, which this spec proves only at single-note granularity). Owner: operator (deferral decided; sequencing to be fixed at plan time).